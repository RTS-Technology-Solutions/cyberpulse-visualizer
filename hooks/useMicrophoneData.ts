  ...
  const requestAndStartMicrophone = useCallback(async () => {
    ...
    try {
      ...
      sourceRef.current = audioContextRef.current.createMediaStreamSource(stream);
        console.log('Audio nodes connected successfully. Initial gain:', MIC_INITIAL_GAIN);
      ...
      const update = () => {
        if (!analyserRef.current || !dataArrayRef.current || !gainNodeRef.current || !audioContextRef.current) {
        ...
        for (let i = 0; i < dataArrayRef.current.length; i++) {
            agcSum += dataArrayRef.current[i];
        }
        const averageVolume = dataArrayRef.current.length > 0 ? agcSum / dataArrayRef.current.length : 0;
