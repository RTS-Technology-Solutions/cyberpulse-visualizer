...
import { CyberPulseSchema } from './components/CyberPulseSchema';
...
const App: React.FC = () => {
  ...
  const [showEQ, setShowEQ] = useState<boolean>(false); // EQ bars off by default
  ...
  const visualizerRenderParams: VisualizerRenderParams = useMemo(() => ({
    punch: visualizerParams.punch,
    vibe: visualizerParams.vibe,
    mood: currentMoodForVisualizer,
    showEQ: showEQ,
  }), [visualizerParams.punch, visualizerParams.vibe, currentMoodForVisualizer, showEQ]);
      ...
      >
        {visualizerIsPlaying && (
          <div className="flex flex-col items-center gap-4">
            <ControlsPanel sliders={activeSliderConfigs} />
            <button
              onClick={() => setShowEQ(!showEQ)}
              className={`px-4 py-2 rounded-lg font-semibold text-sm transition-all duration-300 ease-in-out transform hover:scale-105 ${
                showEQ 
                  ? 'bg-cyber-primary text-cyber-bg shadow-lg shadow-cyber-primary/50' 
                  : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
              }`}
              aria-label="Toggle EQ Display"
            >
              {showEQ ? 'EQ: ON' : 'EQ: OFF'}
            </button>
          </div>
